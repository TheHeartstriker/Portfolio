...
Thankfully this effect is heavily based on the fundementals of the previous one so, not much more code is needed! But first up to makes this we increase `pixSize` from whatever you chose previously to a larger number like 40px. But now there is a problem we have larger cells now and if the container isn't a perfect mutiple of our `pixSize` we have leftover space at the edges where there is not enough room for a new cell causing large visable gaps. This existed before but is made more aprent at larger cells sizes
...
Now the grids edge's or gaps are simply not visable to us anymore because we have in essence zoomed in by increasing its size while that size is just cutoff by the `overflow: hidden` in our css parent card. The issues still exists it just on a area of the canvas thats no longer visable.
