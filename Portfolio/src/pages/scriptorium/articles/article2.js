...
This article is going to map a fullstack system: the what, where, and why. This is not really system design but going over a basic already designed system and aimed at beginear to lower-intermediate web dev's. There are going to be three sections. First is serving code; this will go over what happens when you visit a fullstack application. 
...
The true name is just a random string of numbers that point toward the location of the server/endpoint. So let’s go over how and what a DNS is. DNS is a bit complex. In truth, first, in some cases, they aren’t even used because you have already visited the site and your browser has cached the IP address. 
...
More directly, it ensures all data packets are sent and received in the correct order. A connection exists before data is sent. Verify data integrity. Data flow — making sure we don't send too much data at once. And that’s what it does and why it's used. 
...
TLS is a wrapper for TCP that adds encryption to the data being sent and received. And this is used everywhere — a website with https:// uses TLS and is technically using military-grade encryption. Now, this is rather simple to know why it's used, but let’s go over how it initiates, which is arguably more important than understanding TCP specifics, because you might actually need to know specifics of how TLS works, while TCP is more automated and you don’t need to know the specifics.
...
It’s not some special or magical thing, just some key-value pairs that the client uses to verify information and acquire information.
...
The server checks the credentials against a database, checking if it exists, if the information is correct, etc. And wraps the information with a JWT. I will go over why we don’t store the JWT or information on the backend or database later in the article. But the JWT, while having nuances, is a kind of wrapper for information. It holds 3 important pieces of information. The first is the header, which is the algorithm used to sign the token. The second is the data or information we are wrapping. And the third and most important is the signature, which is a combination of the header and payload that is signed with the secret key.
...
The second is size limitations — it’s vastly smaller than local storage and has a limit of 4KB per cookie, and most websites have a max cookie limit.
The third, and most important, is flags that can be set on a cookie like HttpOnly, Secure, SameSite, Domain, and time. These all have meanings and reasons, but here I am going to talk about time. The rest is a little bit of research you can do on your own.
...
Now, of the available cookies we can use — which is different for each browser — we are using only one, which is wrapped in a JWT.

### III. API Calls
The API calls are the next step. Let's go over requesting data:
First, we send a cookie and information related to the backend function we are calling. If we pass certain checks like authentication, CORS, and other things I will go over later, we get a response back with the included data.
This is the same for all API calls and is the most basic operation in a fullstack application.
...
When you click on a button, for example, we go to another route and unload the previous page and load a new one. It's easy for us to implement and use. Some websites don't have this, and that's fine, but most do.
...
Commonly, this is something called a controller file that holds controllers — functions that handle the request and response. And this is how we organize our backend code.

### V. Database
Now this is an integral part of a website. Most everything on a website specific to you, the user, is stored in a database. We store things like login information, user data, posts, comments, and other things. This is a very important part of a full-stack application. There are many types of databases or methods of storing data.
...
Such as sanitizing inputs, not exposing the database to the internet, using .env files to connect and store the database connection information, limiting types of connections allowed, permission monitoring, and using a TLS connection to the database.

### VI. Backend Protections
We have covered most everything integral to a basic full-stack application—the more external stuff—and here we are going over backend protections. This is very important and often overlooked by beginner developers. But it’s important because if you're doing things correctly, then the backend is the gatekeeper for portions of frontend code and data. So we need to protect it.
...
The second and most important is middleware, often called auth or something similar. This is a function that checks a request's credentials. Do they have a valid JWT? Is it expired? Is it a valid user? And other requests that a programmer might want to check. If I visit a site and go to a protected route, something like an auth middleware will check and, in this case, not allow me to access any HTTP request or even load the component—depending on the developer. Although, as we went over, frontend code does not really do much if someone is determined to get the data.
There are other protections like rate limiting, input sanitization, and other things that are important to know about but not really needed to understand a full-stack application.

Puzzle Pieces
This is a basic high-level overview of a full-stack application and how it works. I did not go over the why of everything because that goes into system design, and that is not what this article is about.
This is about—if someone is building a full-stack application—what are the pieces they need to know about and how they fit together.
Any of these things that I mentioned can be moved around, manipulated, changed, and I have no doubt that some research into any of these topics will lead you to ways of improving your full-stack application.
But no matter what, almost all full-stack applications will have these pieces, and it's important to understand how they fit together.
It gives more freedom of thought and creativity for a developer. And if you know this, abstractions? Are easy. SaaS products? You’ve already made a full-stack application manually or understand what’s actually happening. 
So an abstraction? That’s light work. If you understand this stuff, maybe implement it yourself. Then all full-stack applications are just these pieces implemented in different ways.

Conclusion
I hope you gained something from this article and learned something new. Apart from my bad diagram skills, I hope you learned something about how a full-stack application works and how the pieces fit together. 
